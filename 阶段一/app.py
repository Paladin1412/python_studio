...
while(True):
    ...
    print('├'+'-'*82+'┤')
    for i in range(5):
        print('│'+' '*3+date[i]+' '*4+'│'+' '*3+str(city)+' '*(9-len(str(city)))+'│'+' '*4+str(temp[i])+' '*(8-len(str(temp[i])))+'│'+' '*4+str(temp_min[i])+' '*(10-len(str(temp_min[i])))+'│'+' '*5+str(temp_max[i])+' '*(9-len(str(temp_max[i])))+'│'+' '*3+str(weather[i])+' '*(10-len(str(weather[i])))+'│')
    print('└'+'-'*82+'┘')
