...
def post_request(url, formdata, headers):
    return requests.post(url, headers=headers, data=formdata).content.decode('gbk')
...
def get_page_job(company, url, i, error_list):
    ...
    try:
        ...
        error_list.append([url, "'pageno':{}".format(i), str(e)])  # url，第i页，错误原因
        # write_error_list([url,"'pageno':{}".format(i), e],id)
    return all_job

def get_all_job(company, url, id, all_error_list):
    '''获取当前公司的所有职位信息'''
    all_job = []
    i = 1
    error_list = []
...
def get_all_job_json(start, end, ID):
    ...
    for j in range(start, end):
        ...
        all_job = get_all_job(company_list[j], url_list[j], ID,all_error_list)
    ...
    if all_error_list!=[]:
        write_error_list(all_error_list, ID)
